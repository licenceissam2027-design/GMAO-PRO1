...
class UpdateProjectPhaseRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $projectPhase = $this->route('projectPhase');
        ...
        if (Schema::hasTable('project_phases')) {
            $rules['depends_on_phase_id'][] = Rule::exists('project_phases', 'id')->where(fn ($q) => $q->where('project_id', $projectId));
            $rules['depends_on_phase_id'][] = Rule::notIn([$projectPhase?->id]);
        }
